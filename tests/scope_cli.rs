...
fn init_git_repo(dir: &Path, branch: &str) {
    ...
    // config
    let status = Command::new("git").current_dir(dir).args(["config", "user.name", "Test User"]).status().unwrap();
    assert!(status.success());
    let status = Command::new("git").current_dir(dir).args(["config", "user.email", "test@example.com"]).status().unwrap();
    assert!(status.success());
    ...
    assert!(status.success());
    let status = Command::new("git").current_dir(dir).args(["commit", "-q", "-m", "chore: init"]).status().unwrap();
    assert!(status.success());

    // branch (create or switch)
    let status = Command::new("git").current_dir(dir).args(["switch", "-C", branch]).status().unwrap();
    assert!(status.success());
}
...
fn scope_outputs_minor_on_develop() {
    ...
    run_scope(tmp.path()).success().stdout(predicate::str::is_match("^(minor)\\n$").unwrap());
}
...
fn scope_outputs_major_on_release_branch() {
    ...
    run_scope(tmp.path()).success().stdout(predicate::str::is_match("^(major)\\n$").unwrap());
}
...
fn scope_outputs_patch_on_hotfix_branch() {
    ...
    run_scope(tmp.path()).success().stdout(predicate::str::is_match("^(patch)\\n$").unwrap());
}
...
fn scope_outputs_release_on_main() {
    ...
    run_scope(tmp.path()).success().stdout(predicate::str::is_match("^(release)\\n$").unwrap());
}
