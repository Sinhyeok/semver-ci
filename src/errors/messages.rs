...
pub(crate) const FETCH_COMPLETE_HISTORY: &str = "Failed to fetch complete CI history from origin. Check repository access, or configure a full CI checkout.";
pub(crate) const INCOMPLETE_FETCH: &str = "Repository is still shallow after fetching from origin. Use a remote with complete history, or configure a full CI checkout.";
